...
import { encodeMaskRle, PRESENTATION_STATE_SCHEMA, PRESENTATION_STATE_VERSION, type PresentationState } from '@/state/presentationState';
import { directionFlat } from '@/math/vec3';
import { measure } from '@/measurement/measurements';
...
export async function captureScreenshot(opts: ScreenshotOptions = {}): Promise<Blob> {
  ...
  if (a) {
    ...
    if (a.orientation) {
      const { screenDirections } = await import('@/rendering/camera');
      const { anatomicalLabel } = await import('@/dicom/geometry');
