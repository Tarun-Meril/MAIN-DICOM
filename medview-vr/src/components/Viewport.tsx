...
import { VolumeRenderEngine } from '@/rendering/engine';
import { resolveClipPlanes, resolveCropPlanes, cropBoxWorld } from '@/rendering/clipping';
import { screenDirections, type CameraState } from '@/rendering/camera';
...
import { measure, type MeasurementBase, type MeasurementKind } from '@/measurement/measurements';
import { sculptSphere, sculptPolygonPrism, regionGrow } from '@/segmentation/operations';
import { useAppStore } from '@/state/store';
import { getSession, syncDisplay } from '@/state/session';
import { OrientationCube } from './OrientationCube';
import { logger, scopedLogger } from '@/core/logger';
import { MedViewError } from '@/core/errors';
import type { Vec3 } from '@/math/vec3';
import { normalize, cross, sub } from '@/math/vec3';
...
export function Viewport(): JSX.Element {
  ...
  useEffect(() => {
    ...
    (window as unknown as Record<string, unknown>).__medview = {
      ...
      logger,
      services: {
        captureScreenshot: (o?: unknown) => import('@/services/exportService').then((m) => m.captureScreenshot(o as never)),
        capturePresentationState: () => import('@/services/exportService').then((m) => m.capturePresentationState()),
      },
      ops: {
        segmentation: () => import('@/segmentation/operations'),
        boneRemoval: () => import('@/segmentation/boneRemoval'),
        measurements: () => import('@/measurement/measurements'),
        picking: () => import('@/measurement/picking'),
        surface: () => import('@/rendering/surface'),
      },
    };
