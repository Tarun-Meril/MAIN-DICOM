...
import { CAMERA_PRESETS } from '@3d/rendering/camera';
import { captureScreenshot, capturePresentationState } from '@vr/services/exportService';
import { parsePresentationState, matchesVolume, decodeMaskRle } from '@vr/state/presentationState';
import { downloadBlob, downloadText, slug } from '@vr/utils/download';
import { APP_NAME, APP_VERSION } from '@vr/core/version';

export interface ToolbarProps {
  /** Rendered as "Back to 2D" when the host supplies it. */
  onClose?: () => void;
  /** 'embedded' trims the standalone wordmark to save vertical space. */
  variant?: 'embedded' | 'standalone';
}

export function Toolbar({ onClose, variant = 'standalone' }: ToolbarProps = {}): JSX.Element {
  const s = useAppStore();
  ...
  return (
    <div className="toolbar">
      <div className="brand">
        <b>{APP_NAME}</b>
        {variant === 'standalone' && <span>v{APP_VERSION} · prototype</span>}
        {variant === 'embedded' && <span>VR</span>}
      </div>

      {onClose && (
        <div className="tb-group">
          <button className="tb-btn" onClick={onClose} title="Return to the 2D viewer">← Back to 2D</button>
        </div>
      )}
